...
#
# Usage: git-sync-all.sh [--discard-beads] [directory]
#   directory: Optional path to scan (defaults to ~/Src)
#   --discard-beads: Discard .beads/issues.jsonl changes before pulling
...
DISCARD_BEADS="${DISCARD_BEADS:-0}"
if [[ "${1:-}" == "--discard-beads" ]]; then
    DISCARD_BEADS=1
    shift
fi
...
while IFS= read -r -d '' git_dir; do
    ...
    if cd "$repo_dir" 2>/dev/null; then
        ...
        if [ -n "$(git status --porcelain 2>/dev/null)" ]; then
            ...
            if git status --porcelain 2>/dev/null | grep -Fq ".beads/issues.jsonl"; then
                if [[ "$DISCARD_BEADS" == "1" ]]; then
                    echo -e "${YELLOW}  → Discarding .beads/issues.jsonl changes${NC}"
                    git checkout -- .beads/issues.jsonl 2>/dev/null || true
                else
                    echo -e "${YELLOW}  → .beads/issues.jsonl changed; leaving it untouched${NC}"
                fi
            fi
        fi
        ...
        if output=$(git pull 2>&1); then
            ...
            if [[ "$DISCARD_BEADS" == "1" ]] && echo "$output" | grep -Fq ".beads/issues.jsonl"; then
