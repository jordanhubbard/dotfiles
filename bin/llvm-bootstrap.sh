...
if [ ! -d "${_LLVM_PROJ}" ]; then
	...
	git clone --depth=1 ${_BRANCH} https://github.com/llvm/llvm-project.git || usage "Can't clone LLVM project"
...
cmake -DCMAKE_BUILD_TYPE=Release -DLLVM_ENABLE_PROJECTS='clang;clang-tools-extra;lldb;compiler-rt;lld;polly' ../llvm || usage "Cmake step failed"
